...
// vigies
    $vigies=$conn->query("SELECT * FROM products WHERE status = 1 AND categories_id=1");
    $vigies->execute();

    $allVigies=$vigies->fetchAll(PDO::FETCH_OBJ);
    //meats
    $meats=$conn->query("SELECT * FROM products WHERE status = 1 AND categories_id = 2");
    $meats->execute();

    $allMeats=$meats->fetchAll(PDO::FETCH_OBJ);
//fish
    $fishes=$conn->query("SELECT * FROM products WHERE status = 1 AND categories_id = 3");
    $fishes->execute();

    $allFishes=$fishes->fetchAll(PDO::FETCH_OBJ);
//fruit
    $fruits=$conn->query("SELECT * FROM products WHERE status = 1 AND categories_id = 4");
    $fruits->execute();
